...
public class AbstractDictionaryEntryTest {
    ...
    public void testToString() throws Exception {
        ...
        assertEquals("Санкт-Петербург(2)", Dictionaries2.getInstance().getAreas().getByName("Санкт-Петербург").toString());
        assertEquals("Программирование, Разработка(1.221)",
                Dictionaries2.getInstance().getProfessionalFields().getById("1.221").toString());
        assertEquals("Кольцевая(5)", Dictionaries2.getInstance().getMetroCities().getByName("Кольцевая").toString());
        assertEquals("Чернышевская(14.198)", Dictionaries2.getInstance().getMetroCities().getByName("Чернышевская").toString());
    }
}
